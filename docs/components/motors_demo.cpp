// Motors demo using Zeus Car Shield pins (from sample-component-code.md)
// Drives the four motors forward for 2s, reverse for 2s, then stops.
#include <Arduino.h>
...
namespace softpwm {
  ...
  void begin() {
    cli(); TCCR2A = 0; TCCR2B = 0; TCCR2B |= (1 << CS22); TIMSK2 |= (1 << TOIE2); sei();
  }
  ISR(TIMER2_OVF_vect) {
    phase++;
    for (uint8_t i = 0; i < MAX_CH; i++) if (ch[i].active) digitalWrite(ch[i].pin, (phase < ch[i].duty) ? HIGH : LOW);
  }
}
...
void loop() {
  ...
  if (now - t0 < 2000) {
    drive.setWheels(180, 180, 180, 180); // forward
  } else if (now - t0 < 4000) {
    drive.setWheels(-180, -180, -180, -180); // reverse
  } else {
